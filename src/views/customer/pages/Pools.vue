<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-teal-50/30 to-cyan-50/40 p-4 md:p-8">
    <!-- Loading -->
    <div v-if="loading" class="max-w-6xl mx-auto space-y-6 animate-pulse">
      ...
      <header class="mb-8">
        <div class="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p class="text-gray-500">View your pools and explore service history for each one.</p>
          </div>
        </div>
      </header>

      <!-- Empty -->
      <div v-if="pools.length === 0" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
        <div class="text-6xl mb-4">🏊</div>
        <h3 class="text-xl font-semibold text-gray-800 mb-2">No pools yet</h3>
        <p class="text-gray-500">Your service provider hasn't added any pools to your account.</p>
      </div>
          ...
          <div class="p-6">
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center gap-3">
                <div class="w-14 h-14 rounded-xl bg-teal-50 flex items-center justify-center text-2xl">🏊</div>
                <div>
                  <h2 class="text-xl font-bold text-gray-900">{{ pool.label || pool.name || 'Pool' }}</h2>
                  <p v-if="pool.service_address" class="text-sm text-gray-500 mt-0.5">{{ pool.service_address }}</p>
                </div>
              </div>
              <span
            ...
            <dl class="grid grid-cols-2 gap-3 mb-6">
              <div class="bg-gray-50 rounded-xl p-3">
                <dt class="text-xs text-gray-500 uppercase tracking-wide">Volume</dt>
                <dd class="text-sm font-medium text-gray-800 mt-1">{{ pool.volume_gallons ? `${pool.volume_gallons} gal` : 'Not specified' }}</dd>
              </div>
              <div class="bg-gray-50 rounded-xl p-3">
                <dt class="text-xs text-gray-500 uppercase tracking-wide">Chemical</dt>
                <dd class="text-sm font-medium text-gray-800 mt-1">{{ pool.chemical_type || 'Unknown' }}</dd>
              </div>
              <div class="bg-gray-50 rounded-xl p-3">
                <dt class="text-xs text-gray-500 uppercase tracking-wide">Season</dt>
                <dd class="text-sm font-medium text-gray-800 mt-1">{{ pool.season || 'Not set' }}</dd>
              </div>
              <div class="bg-gray-50 rounded-xl p-3">
                <dt class="text-xs text-gray-500 uppercase tracking-wide">Gate Code</dt>
                <dd class="text-sm font-medium text-gray-800 mt-1">{{ pool.gate_code || 'N/A' }}</dd>
              </div>
              <div class="bg-gray-50 rounded-xl p-3">
                <dt class="text-xs text-gray-500 uppercase tracking-wide">Has Dog</dt>
                <dd class="text-sm font-medium text-gray-800 mt-1">{{ pool.has_dog === true ? 'Yes' : pool.has_dog === false ? 'No' : 'Unknown' }}</dd>
              </div>
              <div class="bg-gray-50 rounded-xl p-3">
                <dt class="text-xs text-gray-500 uppercase tracking-wide">Equipment</dt>
                <dd class="text-sm font-medium text-gray-800 mt-1">{{ pool.equipment?.length || 0 }} items</dd>
              </div>
              <div class="bg-gray-50 rounded-xl p-3 col-span-2">
                <dt class="text-xs text-gray-500 uppercase tracking-wide">Notes</dt>
                <dd class="text-sm font-medium text-gray-800 mt-1">{{ pool.access_notes || 'No access notes available' }}</dd>
              </div>
            </dl>
