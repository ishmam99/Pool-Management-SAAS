<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50/30 to-sky-50/40 p-4 md:p-8">
    <div v-if="loading" class="max-w-6xl mx-auto animate-pulse space-y-6">
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-50">
              <tr v-for="invoice in invoices" :key="invoice.id || invoice.invoice_number" class="hover:bg-gray-50/80 transition">
                ...
                <td class="px-6 py-4 text-sm text-gray-600">{{ formatDate(invoice.invoice_date || invoice.created_at) }}</td>
                <td class="px-6 py-4 text-sm text-gray-600">{{ formatDate(invoice.due_date) }}</td>
                <td class="px-6 py-4 font-semibold text-gray-900">{{ formatCurrency(invoice.total || invoice.amount || invoice.subtotal) }}</td>
                <td class="px-6 py-4 font-semibold text-gray-900">{{ formatCurrency(invoice.balance_due || 0) }}</td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-right">
                  <router-link
                    :to="`/customer/billing/${invoice.id}`"
