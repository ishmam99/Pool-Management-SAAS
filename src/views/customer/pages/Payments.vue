<template>
  <div
    class="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50/30 to-sky-50/40 p-4 md:p-8"
  >
    ...
    <div v-else class="mx-auto">
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-500 to-sky-600 flex items-center justify-center text-white text-xl shadow-lg"
          >
            <i class="ri-bank-card-line"></i>
          </div>
          <div>
            <h1 class="text-3xl font-bold text-gray-900">Payments</h1>
            <p class="text-gray-500">Payments and account balance.</p>
          </div>
        </div>
      </header>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div class="bg-amber-50 rounded-2xl shadow-sm border p-5">
          ...
          <div
            ...
            :class="summary.balanceDue > 0 ? 'text-red-600' : 'text-gray-800'"
          >
      ...
      <div class="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full table-zebra">
            <thead
              class="!bg-sky-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500"
            >
              ...
              <th class="px-6 py-3 border-e border-slate-300 text-right">
                Actions
              </th>
            </thead>
              ...
              >
                <td class="px-6 py-4 border-b border-e border-slate-300">
                  ...
                  <div class="text-xs text-gray-500">
                    Agreement #{{ invoice.service_agreement_id || "—" }}
                  </div>
                </td>
                <td
                  class="px-6 py-4 border-b border-e border-slate-300 text-sm text-gray-600"
                >
                ...
                <td
                  class="px-6 py-4 border-b border-e border-slate-300 font-semibold text-gray-900"
                >
                  {{
                    formatCurrency(
                      invoice.total || invoice.amount || invoice.subtotal,
                    )
                  }}
                </td>
                <td
                ...
                <td
                  class="px-6 py-4 border-b border-e border-slate-300 text-right"
                >
                  <router-link
                    :to="{
                      path: `/customer/billing/${invoice.id}`,
                      query: { type: 'payment' },
                    }"
...
const summary = computed(() => {
  const totalPaid = invoices.value.reduce(
    (sum, invoice) => sum + Number(invoice.amount_paid || 0),
    0,
  );
  const balanceDue = invoices.value.reduce(
...
const fetchInvoices = async () => {
  ...
  try {
    ...
    invoices.value = Array.isArray(data)
      ? data.filter((invoice) => invoice.status == "paid")
      : data?.data.filter((invoice) => invoice.status == "paid") ||
