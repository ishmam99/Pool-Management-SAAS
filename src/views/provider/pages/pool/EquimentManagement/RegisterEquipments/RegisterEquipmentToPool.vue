<template>
  <div class="min-h-screen bg-gray-50 font-inter">
    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div v-if="showInstallForm" class="rounded-2xl border border-cyan-200 overflow-hidden bg-white shadow-lg shadow-cyan-500/5">
        <div class="bg-gradient-to-r from-cyan-50 via-blue-50 to-white px-6 py-4 border-b border-cyan-200">
          <div class="flex items-center justify-between">
            ...
            <button @click="cancelInstallation" class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-200 transition-all">
        ...
        <div class="p-6 space-y-5">
          ...
          <div>
            ...
            <select v-model="installForm.manufacturer_id" @change="onManufacturerChange" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="loadingManufacturers || installing">
          ...
          <div>
            ...
            <select v-model="installForm.equipment_model_id" @change="onModelChange" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="!installForm.manufacturer_id || loadingModels || installing">
          ...
          <div v-if="installForm.equipment_model_id && catalogComponents.length > 0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-1">
              <div v-for="comp in catalogComponents" :key="comp.id" @click="toggleComponent(comp.id)" class="p-4 rounded-xl border-2 transition-all cursor-pointer" :class="[selectedComponentIds.includes(comp.id) ? 'border-cyan-500 bg-cyan-50/50' : 'border-gray-200 bg-gray-50 hover:border-gray-300']">
                <div class="flex items-start gap-3">
                  <input type="checkbox" :checked="selectedComponentIds.includes(comp.id)" class="mt-1 w-4 h-4 text-cyan-500 border-gray-300 rounded focus:ring-cyan-500 pointer-events-none">
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between gap-2">
                      <span class="text-sm font-semibold text-gray-900 truncate">{{ comp.name }}</span>
                      <span :class="[comp.is_required ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-gray-100 text-gray-600 border-gray-200', 'inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border flex-shrink-0']">
                    ...
                    <div class="flex items-center gap-2 mt-1.5 text-xs text-gray-500">
                      ...
                      <span>|</span>
                      <span class="font-semibold text-gray-700">${{ comp.default_price?.toLocaleString() || '0' }}</span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div v-if="selectedComponentIds.length === 0" class="mt-2 text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5">
          ...
          <div v-if="selectedComponentIds.length > 0" class="space-y-4">
            <div class="flex items-center gap-2">
              ...
              <span class="ml-auto text-xs text-gray-500">{{ selectedComponentIds.length }} component{{ selectedComponentIds.length > 1 ? 's' : '' }} selected</span>
            </div>
            <div v-for="compId in selectedComponentIds" :key="compId" class="p-4 rounded-xl bg-gray-50 border border-gray-200">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input v-model="componentDetails[compId].serial_number" type="text" placeholder="Enter serial number" class="w-full px-3.5 py-2 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="installing">
                </div>
                <div>
                  <label class="block text-xs font-semibold text-gray-600 mb-1">Install Date</label>
                  <input v-model="componentDetails[compId].install_date" type="date" class="w-full px-3.5 py-2 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="installing">
                </div>
                <div>
                  <label class="block text-xs font-semibold text-gray-600 mb-1">Warranty Expiry</label>
                  <input v-model="componentDetails[compId].warranty_expiry" type="date" class="w-full px-3.5 py-2 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="installing">
                </div>
                <div>
                  <label class="block text-xs font-semibold text-gray-600 mb-1">Purchase Price</label>
                  <input v-model="componentDetails[compId].purchase_price" type="number" step="0.01" class="w-full px-3.5 py-2 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="installing">
                </div>
                <div class="col-span-2">
                  <label class="block text-xs font-semibold text-gray-600 mb-1">Notes</label>
                  <textarea v-model="componentDetails[compId].notes" rows="2" placeholder="Additional notes..." class="w-full px-3.5 py-2 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="installing"></textarea>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
              <button @click="saveInstallation" :disabled="installing || !isInstallFormValid" class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 transition-all hover:-translate-y-0.5">
                <svg v-if="installing" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                {{ installing ? 'Installing...' : 'Install Equipment' }}
              </button>
              <button @click="cancelInstallation" class="px-6 py-2.5 rounded-xl text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-200 transition-all" :disabled="installing">Cancel</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-else-if="!loadingInstalled && installedEquipment.length === 0" class="flex flex-col items-center justify-center py-16 rounded-2xl border border-gray-200 bg-white shadow-sm">
          ...
          <button v-if="!showInstallForm" @click="showInstallForm = true" class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/25 transition-all hover:-translate-y-0.5">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            Install Equipment
          </button>
        </div>
        ...
        <div v-if="editComponentModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-lg bg-white border border-gray-200 rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div class="p-6 space-y-4">
              <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Serial Number</label>
                <input v-model="editForm.serial_number" type="text" class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="updating">
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Install Date</label>
                <input v-model="editForm.install_date" type="date" class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="updating">
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Warranty Expiry</label>
                <input v-model="editForm.warranty_expiry" type="date" class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="updating">
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Purchase Price</label>
                <input v-model="editForm.purchase_price" type="number" step="0.01" class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="updating">
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Status</label>
                <select v-model="editForm.status" class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="updating">
              ...
              <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Notes</label>
                <textarea v-model="editForm.notes" rows="3" placeholder="Additional notes..." class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all" :disabled="updating"></textarea>
              </div>
            </div>
            <div class="flex items-center gap-3 px-6 py-4 border-t border-gray-200">
              <button @click="updateComponent" :disabled="updating || !editForm.serial_number" class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 transition-all hover:-translate-y-0.5">
                <svg v-if="updating" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                {{ updating ? 'Updating...' : 'Update Component' }}
              </button>
              <button @click="closeEditComponentModal" class="px-6 py-2.5 rounded-xl text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-200 transition-all" :disabled="updating">Cancel</button>
            </div>
          </div>
        </div>
      </Transition>
