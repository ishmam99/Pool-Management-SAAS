<template>
  <div class="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">Add Terms and Conditions for {{ pageTitle }}</h1>
        ...
        <table class="w-full table-auto table-zebra">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="filteredCustomers.length === 0">
              <td colspan="8" class="px-4 py-12 text-center">
                <div class="flex flex-col items-center">
                  ...
                  <button @click="router.push('/provider/customer-create')"
            ...
            <tr v-else v-for="customer in paginatedCustomers" :key="customer.id"
              ...
              <td class="whitespace-nowrap border-e border-slate-200 px-4 py-3">
                <div class="flex items-center space-x-3">
                  ...
                  <router-link :to="getCustomerDetailsRoute(customer)">
                    <p class="text-sm font-medium text-blue-900 underline underline-offset-2">{{ customer.contact_name
                      }}</p>
              ...
              <td class="px-4 py-3 border-e border-slate-200">
                <div class="flex flex-col space-y-1">
                  ...
                  <div v-if="customer.agreements && customer.agreements.length > 0" class="text-xs text-gray-500">
                    {{ customer.agreements[0].frequency }} · ${{ customer.agreements[0].price }}/{{
                      customer.agreements[0].billing_cycle }}
                  </div>
                </div>
              </td>
              <!-- Monthly Value -->
              ...
              <!-- Actions -->
              <td class="whitespace-nowrap border-e border-slate-200 px-4 py-3">
                <router-link :to="`/provider/customer-add-terms/${customer.id}`"
                  class="group flex w-full items-center gap-3 px-4 py-3 text-sm font-medium text-gray-700 transition-all duration-200 hover:bg-indigo-50 hover:text-indigo-700 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                  <!-- Icon with group hover effect -->
                  <span
                    class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 text-gray-500 transition-all duration-200 group-hover:bg-indigo-100 group-hover:text-indigo-600">
                  ...
                  <span class="flex flex-1 items-center justify-between">
                    ...
                    <svg
                      class="h-4 w-4 -translate-x-1 opacity-0 transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100"
        ...
        <div class="relative w-full max-w-3xl rounded-2xl bg-white p-6 shadow-2xl">
          ...
          <div v-if="viewingCustomer" class="space-y-6">
            <div class="flex items-center space-x-4 border-b border-gray-200 pb-4">
              <div
                class="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-blue-100 to-blue-200 text-2xl font-semibold text-blue-700">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ capitalizeFirstLetter(viewingCustomer.preferred_contact) ||
                  'N/A' }}</p>
              </div>
              <div>
    ...
    <div v-if="showViewAgreementsModal" class="fixed inset-0 z-50 overflow-y-auto"
      @click.self="closeViewAgreementsModal">
      <div class="flex min-h-screen items-center justify-center p-4">
        <div class="relative w-full max-w-4xl rounded-2xl bg-white p-6 shadow-2xl">
          <!-- Modal Header -->
          <div class="mb-6 flex items-center justify-between">
...
const customerType = computed(() => route.params.id || 'prospective')
