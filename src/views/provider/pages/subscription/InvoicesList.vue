<template>
    <div class="space-y-6 p-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex gap-3">
                <button @click="downloadAllInvoices" 
                        class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                ...
                <button @click="refreshInvoices" :disabled="loading"
                        class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                    <svg class="w-4 h-4 mr-2" :class="{ 'animate-spin': loading }" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex items-start">
                <div class="flex-shrink-0">
                    <svg class="h-5 w-5 text-red-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L10 8.586l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
                    </svg>
                </div>
                <div class="ml-3">
        ...
        <div v-if="!error" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <StatCard v-for="stat in statistics" :key="stat.label" :label="stat.label" :value="stat.value" :loading="loading" />
        </div>
        ...
        <div v-if="!error" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="lg:col-span-2">
                    ...
                    <div class="relative">
                        <input v-model="filters.search" type="text" placeholder="Invoice number or period..."
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                               :disabled="loading" @input="applyFilters" />
                ...
                <div>
                    ...
                    <select v-model="filters.status" @change="applyFilters"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                            :disabled="loading">
                ...
                <div>
                    ...
                    <select v-model="filters.billing_model" @change="applyFilters"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                            :disabled="loading">
        ...
        <div v-if="!error" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="hidden md:block overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr v-else v-for="invoice in invoices" :key="invoice.id" class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm font-medium text-gray-900">{{ invoice.invoice_number }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm text-gray-600">{{ invoice.billing_period || 'N/A' }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 capitalize">
                                    {{ invoice.billing_model?.replace('_', ' ') || 'N/A' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm font-medium text-gray-900">{{ formatCurrency(invoice.total) }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="md:hidden divide-y divide-gray-200">
                ...
                <div v-else v-for="invoice in invoices" :key="invoice.id" class="p-4 hover:bg-gray-50 transition-colors">
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            <div class="text-sm font-medium text-gray-900">{{ invoice.invoice_number }}</div>
                            <div class="text-sm text-gray-600">{{ invoice.billing_period || 'N/A' }}</div>
                        </div>
                        <StatusBadge :status="invoice.status" />
                    </div>
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        <div>
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 ml-1 capitalize">
                                {{ invoice.billing_model?.replace('_', ' ') || 'N/A' }}
                            </span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <span class="text-gray-900 ml-1">{{ formatDate(invoice.created_at) }}</span>
                        </div>
                    </div>
                    <div class="flex items-center justify-end gap-2 mt-3 pt-3 border-t border-gray-200 flex-wrap">
                        <button @click="openInvoiceDetail(invoice)"
                            class="px-3 py-1.5 text-sm text-indigo-600 border border-indigo-200 hover:bg-indigo-50 rounded-lg transition-colors">
                            View
                        </button>
                        <button @click="generateInvoicePDF(invoice)"
        ...
        <!-- ========================================================== -->
        <!-- INVOICE DETAIL DRAWER - Uses InvoiceViewer Component        -->
        <!-- ========================================================== -->
                ...
                <div class="relative bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
                    ...
                    <div class="space-y-4">
                        <div class="bg-gray-50 rounded-lg p-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">{{ formatCurrency(paymentInvoice?.total) }}</p>
                        ...
                        <div v-if="paymentMethod === 'stripe'" class="border border-gray-200 rounded-lg p-4">
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <input type="text" v-model="cardNumber" placeholder="4242 4242 4242 4242" 
...
import StatusBadge from '@/views/admin/components/StatusBadge.vue'
import StatCard from '@/views/admin/components/StatCard.vue'
import InvoiceViewer from '@/views/admin/components/InvoiceViewer.vue'
...
const fetchInvoices = async () => {
    ...
    try {
        ...
        console.log(params)
        const response = await api().get('/tenant/my-invoices', { params })
...
const confirmPayment = async () => {
    ...
    try {
        await api().post(`/tenant/my-invoices/${paymentInvoice.value.id}/pay`, {
...
const generateInvoicePDF2 = (invoice) => {
    if (!invoice) return

    // Import jsPDF dynamically or use existing import
    // For now, we'll use the same PDF generation as admin
    try {
        // You can either:
        // 1. Use the same PDF generation logic from admin
        // 2. Call an API endpoint that returns the PDF
        // 3. Use a library like html2pdf
        
        // Option 1: API download
        api().get(`/tenant/my-invoices/${invoice.id}/download`, {
            responseType: 'blob'
        }).then(response => {
...
const downloadAllInvoices = () => {
    Swal.fire('Info', 'Download all invoices feature coming soon', 'info')
}

const sendReminder = async (invoice) => {
    try {
        await api().post(`/tenant/my-invoices/${invoice.id}/remind`)
