      ...
      <div class="relative max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-xl lg:text-2xl font-bold text-gray-900 tracking-tight">Pool Maintenance Recommendations</h1>
              <p class="text-sm text-gray-500 mt-0.5">Create bulk maintenance recommendations for your pools.</p>
            </div>
          </div>
          <!-- ===== CREATE RECOMMENDATIONS BUTTON ===== -->
          <button 
            @click="openRecommendationModal" 
            :disabled="!hasSelectedEquipment || isSubmitting"
            class="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/25 transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
          >
    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div>
        <div class="flex items-center gap-2 mb-4">
          ...
          <span v-if="hasSelectedEquipment" class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
        ...
        <div v-else-if="!loadingInstalled && installedEquipment.length === 0" class="flex flex-col items-center justify-center py-16 rounded-2xl border border-gray-200 bg-white shadow-sm">
          ...
          <p class="text-sm text-gray-500 text-center max-w-xs">Install equipment components to this pool to get started.</p>
        </div>

        <!-- Categorized Tables -->
        <div v-else-if="!loadingInstalled && installedEquipment.length > 0" class="space-y-6">
          <div v-for="category in categorizedSections" :key="category.key" class="rounded-2xl border border-gray-200 overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow duration-200">
            <!-- Section Header -->
              ...
              <table class="w-full text-sm border-collapse">
                <thead class="sticky top-0 z-10">
                  <tr class="bg-gray-50 border-b border-gray-200">
                    <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider border-r border-gray-100 w-12">
                      <input 
                        type="checkbox" 
                    ...
                    <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Estimated Cost</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                  <tr v-for="item in category.items" :key="item.id" class="hover:bg-cyan-50/30 transition-colors">
                    ...
                    <td class="px-4 py-3 border-r border-gray-100 whitespace-nowrap">
                      <select 
                        v-model="recommendationData[item.id].type"
                        :disabled="!selectedEquipmentIds.includes(item.id)"
                        :class="[
                          'w-32 px-2 py-1 border rounded-lg text-sm transition-all',
                    ...
                    <td class="px-4 py-3 whitespace-nowrap">
                      <input 
                        ...
                        step="0.01" 
                        min="0"
                        v-model="recommendationData[item.id].cost"
                        :disabled="!selectedEquipmentIds.includes(item.id)"
                        :class="[
                          'w-28 px-2 py-1 border rounded-lg text-sm transition-all',
                          selectedEquipmentIds.includes(item.id) 
                            ? 'border-gray-300 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/50 bg-white' 
                            : 'border-gray-200 bg-gray-50 text-gray-400 cursor-not-allowed'
                        ]"
          ...
          <div class="relative w-full max-w-lg bg-white border border-gray-200 rounded-3xl shadow-2xl overflow-hidden">
            <div class="bg-gradient-to-r from-gray-50 to-white px-6 py-5 border-b border-gray-200">
              <div class="flex items-center justify-between">
                <div>
                  <h2 class="text-lg font-bold text-gray-900">Create Recommendations</h2>
                  <p class="text-xs text-gray-500 mt-0.5">{{ selectedEquipmentIds.length }} equipment(s) selected</p>
                </div>
            ...
            <div class="p-6 space-y-4">
              <div>
                ...
                <input 
                  ...
                  type="text" 
                  placeholder="e.g., Summer Equipment Maintenance"
              ...
              <div class="bg-gray-50 rounded-xl p-3 border border-gray-200">
                <p class="text-xs font-semibold text-gray-600 mb-2">Selected Equipment Summary</p>
                <div class="max-h-32 overflow-y-auto space-y-1">
                  <div v-for="id in selectedEquipmentIds" :key="id" class="text-xs text-gray-700 flex justify-between">
                    <span>{{ getEquipmentName(id) }}</span>
                    <span class="font-medium">{{ getRecommendationTypeLabel(recommendationData[id]?.type) }} - ${{ recommendationData[id]?.cost || '0.00' }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-200">
              ...
              <button 
                @click="submitRecommendations" 
                :disabled="isSubmitting || !isModalFormValid"
                class="flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/25 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 transition-all hover:-translate-y-0.5"
              >
...
async function submitRecommendations() {
  ...
  // Validate all selected equipments have type and cost
  ...
  const items = selectedEquipmentIds.value.map(id => {
    ...
    return {
      ...
      item_type: recommendationData[id].type, // This is the per-equipment recommendation type
