<template>
  <div class="min-h-screen bg-slate-50 font-inter">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="flex items-center gap-2">
                <h1 class="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight">Chemical Usage</h1>
              ...
              <p class="text-sm text-slate-500 mt-0.5">Track chemical additions and usage for your pools</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div v-for="pool in paginatedPools" :key="pool.id" class="relative">
              ...
              >
                ...
                <div class="col-span-2 flex items-center justify-end gap-2 transition-all duration-200" @click.stop>
                  ...
                  <button @click="openUsageModal(pool)" class="flex items-center justify-center w-9 h-9 rounded-xl bg-violet-100 text-violet-700 border border-violet-200 hover:bg-violet-200 hover:text-violet-800 transition-all duration-200" title="Chemical Usage">
      ...
      <div v-else-if="filteredPools.length > 0" class="md:hidden space-y-3">
        ...
        >
          ...
          <div class="flex items-center gap-2 pt-3 border-t border-slate-100" @click.stop>
            ...
            <button @click="openUsageModal(pool)" class="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-violet-100 text-violet-800 hover:bg-violet-200 hover:text-violet-900 border border-violet-200 transition-all">
    ...
    <!-- ===== CHEMICAL USAGE MODAL ===== -->
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="usageModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div class="absolute inset-0 bg-slate-900/70 backdrop-blur-sm" @click="usageModal = false"></div>
          <div class="relative w-full max-w-5xl bg-white border border-slate-200 rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <!-- Header -->
            <div class="relative bg-gradient-to-r from-violet-50 via-purple-50 to-slate-50 px-6 py-5 border-b border-slate-200 shrink-0">
              <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex items-start gap-4">
                  <div class="flex items-center justify-center w-14 h-14 rounded-3xl bg-gradient-to-br from-violet-500/20 to-purple-500/20 border border-violet-200 shadow-sm">
                    <svg class="w-6 h-6 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                    </svg>
                  </div>
                  <div>
                    <h2 class="text-2xl font-bold text-slate-900">Chemical Usage</h2>
                    <p class="text-sm text-slate-500 mt-1">{{ usagePool?.label || usagePool?.name }} · #{{ usagePool?.id }}</p>
                  </div>
                </div>
                <button @click="usageModal = false" class="flex items-center justify-center w-10 h-10 rounded-2xl text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-all">
              ...
              <div class="mt-5 grid grid-cols-1 lg:grid-cols-[1.2fr_auto] gap-3 items-end">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input v-model="usageFilters.date_from" type="date" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                  </div>
                  <div>
                    <label class="block text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500 mb-2">To</label>
                    <input v-model="usageFilters.date_to" type="date" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
                  </div>
                </div>
                <button
                  @click="fetchChemicalUsage(1)"
                  :disabled="usageLoading"
                  class="w-full sm:w-auto inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-violet-600 to-purple-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-violet-500/20 hover:from-violet-700 hover:to-purple-700 disabled:cursor-not-allowed disabled:opacity-50 transition-all"
                >
                  {{ usageLoading ? 'Loading...' : 'Apply Filter' }}
                </button>
              </div>
            </div>

            <div class="flex-1 overflow-y-auto p-6 space-y-4">
              <div v-if="usageLoading" class="space-y-4">
                <div v-for="i in 3" :key="i" class="h-28 rounded-3xl bg-slate-100 animate-pulse"></div>
              </div>

              <div v-else-if="chemicalUsage.length === 0" class="rounded-3xl border border-slate-200 bg-slate-50 p-10 text-center">
                <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-violet-100 text-violet-600">
                ...
                <h3 class="text-lg font-semibold text-slate-900 mb-2">No chemical usage found</h3>
                <p class="text-sm text-slate-500">Try widening the date range or selecting another pool.</p>
              </div>

              <div v-else class="space-y-4">
                <article
                  v-for="item in chemicalUsage"
                  :key="item.id"
                ...
                >
                  <div class="px-6 py-5">
                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                      <div class="flex items-start gap-4">
                        <div class="flex items-center justify-center w-12 h-12 rounded-2xl bg-violet-50 border border-violet-100 shrink-0">
                          <svg class="w-5 h-5 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/>
                          </svg>
                        </div>
                        <div>
                          <h3 class="text-base font-bold text-slate-900 capitalize">{{ formatUsageChemicalType(item.chemical_type) }}</h3>
                          <p class="text-sm text-slate-500 mt-1">{{ formatDateTime(item.created_at) }}</p>
                          <div class="flex flex-wrap gap-3 text-xs text-slate-500 mt-2">
                            <span v-if="item.work_order_id">Work Order #{{ item.work_order_id }}</span>
                            <span v-if="item.work_order?.pool?.label">Pool: {{ item.work_order.pool.label }}</span>
                          </div>
                        </div>
                      </div>
                      <div class="flex flex-col items-start lg:items-end gap-2 shrink-0">
                        <div class="text-2xl font-bold text-violet-700">
                          {{ item.quantity }} <span class="text-sm font-semibold text-slate-500">{{ item.unit }}</span>
                        </div>
                        <span :class="['inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border capitalize', getReasonStyle(item.reason)]">
            ...
            <div v-if="usagePagination.total > 0" class="shrink-0 px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-3">
              <span class="text-sm text-slate-500">
                {{ usagePagination.total }} record{{ usagePagination.total === 1 ? '' : 's' }} total
              </span>
              <div class="flex items-center gap-2">
                <button
                  @click="fetchChemicalUsage(usagePagination.current_page - 1)"
                  :disabled="usagePagination.current_page <= 1 || usageLoading"
                ...
                <span class="text-xs text-slate-500">Page {{ usagePagination.current_page }} of {{ usagePagination.last_page }}</span>
                <button
                  @click="fetchChemicalUsage(usagePagination.current_page + 1)"
                  :disabled="usagePagination.current_page >= usagePagination.last_page || usageLoading"
                  class="rounded-2xl px-3 py-2 text-xs font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 transition"
                >
                  Next
                </button>
...
const usagePool = ref(null)
const poolToDelete = ref(null)
const isAdding = ref(false)

const chemicalUsage = ref([])
const usageLoading = ref(false)
const usagePagination = ref({ current_page: 1, last_page: 1, total: 0 })
...
async function fetchChemicalUsage(page = 1) {
  if (!usagePool.value?.id) return
  usageLoading.value = true
  try {
    const params = new URLSearchParams({
      pool_id: usagePool.value.id,
      date_from: usageFilters.date_from || getDefaultDateFrom(),
      date_to: usageFilters.date_to || getDefaultDateTo(),
      page: String(page),
    })
    const response = await apiClient.get(`/chemical-management/chemicals-added?${params}`)
    const payload = response.data
    chemicalUsage.value = payload?.data || []
    usagePagination.value = {
      current_page: payload?.current_page || 1,
      last_page: payload?.last_page || 1,
      total: payload?.total || chemicalUsage.value.length,
    }
  } catch (error) {
    console.error('Error fetching chemical usage:', error)
    chemicalUsage.value = []
    usagePagination.value = { current_page: 1, last_page: 1, total: 0 }
    alert(error.response?.data?.message || 'Failed to load chemical usage.')
  } finally {
    usageLoading.value = false
  }
}

function openUsageModal(pool) {
  usagePool.value = pool
  usageFilters.date_from = getDefaultDateFrom()
  usageFilters.date_to = getDefaultDateTo()
  chemicalUsage.value = []
  usagePagination.value = { current_page: 1, last_page: 1, total: 0 }
  usageModal.value = true
  fetchChemicalUsage(1)
}

function formatUsageChemicalType(type) {
  if (!type) return 'Unknown Chemical'
...
function getReasonStyle(reason) {
  const map = {
    ...
    shock: 'bg-amber-50 text-amber-700 border-amber-200',
    algae_treatment: 'bg-orange-50 text-orange-700 border-orange-200',
    other: 'bg-slate-50 text-slate-700 border-slate-200',
  }
  return map[reason] || 'bg-violet-50 text-violet-700 border-violet-200'
}
