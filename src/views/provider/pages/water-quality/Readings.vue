<template>
  <div class="min-h-screen bg-slate-50 font-inter">
    <!-- ===== PAGE HEADER ===== -->
    <div class="relative overflow-hidden bg-gradient-to-br from-slate-100 via-blue-50 to-slate-100 border-b border-slate-200">
      ...
      <div class="relative max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div class="flex items-center gap-3">
            <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-blue-500/20 border border-blue-500/30 backdrop-blur-sm">
            ...
            <div>
              <div class="flex items-center gap-2">
                <h1 class="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight">Water Quality Readings</h1>
                <span class="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 border border-emerald-200">
                  <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                  Live
                </span>
              </div>
              <p class="text-sm text-slate-500 mt-0.5">Monitor your pool water quality readings</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="relative flex-1">
              ...
              <input
                v-model="searchQuery"
                type="text"
                placeholder="Search pools, customers..."
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
              ...
              <div v-if="activeFilterCount > 0" class="flex items-center gap-2 pl-1.5">
                ...
                <button @click="clearFilters" class="text-xs font-medium text-blue-600 hover:text-blue-700 transition-colors">
                  Clear
                </button>
              </div>
            </div>
          ...
          <div class="divide-y divide-slate-100">
            <div v-for="pool in paginatedPools" :key="pool.id" class="relative">
              ...
              >
                ...
                <!-- Actions -->
                <div class="col-span-2 flex items-center justify-end gap-2 transition-all duration-200" @click.stop>
                  <button @click="openViewModal(pool)" class="flex items-center justify-center w-9 h-9 rounded-xl bg-slate-100 text-slate-600 border border-slate-200 hover:bg-blue-100 hover:text-blue-700 hover:border-blue-300 transition-all duration-200" title="View">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                  </button>
                  <button @click="openReadingsModal(pool)" class="flex items-center justify-center w-9 h-9 rounded-xl bg-teal-100 text-teal-700 border border-teal-200 hover:bg-teal-200 hover:text-teal-800 transition-all duration-200" title="Chemical Readings Log">
      ...
      <div v-else-if="filteredPools.length > 0" class="md:hidden space-y-3">
        ...
        >
          <div class="flex items-start justify-between mb-3">
            <div class="flex items-center gap-3">
              <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500/20 to-cyan-500/20 border border-blue-200">
                <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7c0 0 2-2 5-2s5 2 8 2 5-2 5-2v10c0 0-2 2-5 2s-5-2-8-2-5 2-5 2V7z"/>
                </svg>
              </div>
          ...
          <div class="flex items-center gap-2 pt-3 border-t border-slate-100" @click.stop>
            <button @click="openViewModal(pool)" class="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-blue-50 text-blue-700 hover:bg-blue-100 hover:text-blue-900 border border-blue-200 transition-all">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
              View
            </button>
            <button @click="openReadingsModal(pool)" class="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-teal-100 text-teal-800 hover:bg-teal-200 hover:text-teal-900 border border-teal-200 transition-all">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
              Readings
            </button>
          </div>
      ...
      <div v-else class="flex flex-col items-center justify-center py-20 px-4">
        <div class="relative mb-6">
          ...
          <div class="relative flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500/20 to-cyan-500/20 border border-blue-200">
            <svg class="w-10 h-10 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7c0 0 2-2 5-2s5 2 8 2 5-2 5-2v10c0 0-2 2-5 2s-5-2-8-2-5 2-5 2V7z"/>
            </svg>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <button v-if="searchQuery || activeFilter !== 'all'" @click="clearFilters" class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-600 bg-white hover:bg-slate-50 border border-slate-200 transition-all shadow-sm">
            Clear Filters
          </button>
          <button @click="openAddModal" class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 shadow-lg shadow-blue-500/25 transition-all hover:-translate-y-0.5">
      ...
      <div v-if="filteredPools.length > 0" class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-3 text-sm text-slate-500">
          <span>Showing <span class="text-slate-700 font-medium">{{ paginationStart }}–{{ paginationEnd }}</span> of <span class="text-slate-700 font-medium">{{ filteredPools.length }}</span> pools</span>
          <span class="w-px h-4 bg-slate-300"></span>
          <div class="flex items-center gap-2">
            <span>Per page:</span>
            <select v-model="perPage" class="bg-white border border-slate-200 rounded-lg px-2 py-1 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/50">
        ...
        <div class="flex items-center gap-1">
          <button @click="currentPage = Math.max(1, currentPage - 1)" :disabled="currentPage === 1" class="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-slate-600 hover:text-slate-800 bg-white border border-slate-200 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm">
          ...
          <button @click="currentPage = Math.min(totalPages, currentPage + 1)" :disabled="currentPage === totalPages" class="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-slate-600 hover:text-slate-800 bg-white border border-slate-200 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm">
            Next
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="viewModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-2xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden">
            <!-- Modal Header -->
            <div class="relative bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-5 border-b border-slate-200">
              <div class="flex items-start justify-between">
                <div class="flex items-center gap-4">
                  <div class="flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500/30 to-cyan-500/30 border border-blue-200">
                ...
                <button @click="viewModal = false" class="flex items-center justify-center w-8 h-8 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-200 transition-all">
            ...
            <div class="p-6 overflow-y-auto max-h-[70vh] space-y-4">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  <div class="flex items-center gap-2 mb-2">
                    <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18"/></svg>
                    <span class="text-xs text-slate-500 font-medium">Chemical Type</span>
                  </div>
                  <span :class="['inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold border', getChemicalStyle(selectedPool?.chemical_type || selectedPool?.chemicalType)]">
                    <span :class="['w-1.5 h-1.5 rounded-full', getChemicalDot(selectedPool?.chemical_type || selectedPool?.chemicalType)]"></span>
                ...
                <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  ...
                  <div class="text-lg font-bold text-slate-800">{{ formatVolume(selectedPool?.volume_gallons || selectedPool?.volume) }} <span class="text-sm font-normal text-slate-500">gallons</span></div>
                </div>
              ...
              <!-- Access Info -->
              <div v-if="selectedPool?.gate_code || selectedPool?.access_notes" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div v-if="selectedPool?.has_dog !== undefined" class="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  <div class="flex items-center gap-2 mb-2">
                    <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50">
              <button @click="openEditModal(selectedPool); viewModal = false" class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 shadow-lg shadow-blue-500/20 transition-all hover:-translate-y-0.5">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                Edit Pool
              </button>
              <button @click="viewModal = false" class="px-6 py-2.5 rounded-xl text-sm font-semibold text-slate-600 bg-white hover:bg-slate-50 border border-slate-200 transition-all shadow-sm">
                Close
              </button>
            </div>
          </div>
          ...
          <div class="relative w-full max-w-xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden">
            <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-5 border-b border-slate-200">
              <div class="flex items-center justify-between">
                ...
                <button @click="editModal = false" class="flex items-center justify-center w-8 h-8 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-200 transition-all">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
              </div>
            </div>
            ...
            <div class="p-6 overflow-y-auto max-h-[70vh] space-y-5">
              ...
              <div>
                <div class="flex items-center gap-2 mb-3">
                  <div class="w-4 h-4 rounded bg-blue-500/20 border border-blue-500/40 flex items-center justify-center">
                ...
                <div class="space-y-3">
                  <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Pool Name <span class="text-rose-500">*</span></label>
              ...
              <div>
                <div class="flex items-center gap-2 mb-3">
                  <div class="w-4 h-4 rounded bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="col-span-2">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                      <button
                        v-for="type in ['chlorine', 'salt', 'bromine']"
                        :key="type"
                        @click="editForm.chemical_type = type"
                        :class="[
                  ...
                  <div>
                    ...
                    <input v-model="editForm.equipment" type="number" placeholder="0" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all" />
                  </div>
                  <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Season</label>
                    <select v-model="editForm.season" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all">
                  ...
                  <div class="col-span-2">
                    ...
                    <div class="flex rounded-xl border border-slate-200 overflow-hidden bg-white">
                      <button
                        v-for="s in [true, false]"
                        :key="s"
                        @click="editForm.is_active = s"
                        :class="[
              ...
              <div>
                <div class="flex items-center gap-2 mb-3">
                  <div class="w-4 h-4 rounded bg-amber-500/20 border border-amber-500/40 flex items-center justify-center">
                ...
                <div class="space-y-3">
                  <div>
                    ...
                    <input v-model="editForm.service_address" type="text" placeholder="123 Pool Street, City, State" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all" />
                  </div>
                  <div class="grid grid-cols-2 gap-3">
                    <div>
                      <label class="block text-xs font-semibold text-slate-600 mb-1.5">Gate Code</label>
                      <input v-model="editForm.gate_code" type="text" placeholder="e.g. 1234" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all" />
                    </div>
                    <div class="flex items-end">
                      <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                        <input v-model="editForm.has_dog" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                        Has Dog on Property
                      </label>
                    </div>
                  </div>
                  <div>
                    ...
                    <textarea v-model="editForm.access_notes" rows="2" placeholder="Special instructions, access code, etc..." class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all resize-none"></textarea>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50">
              ...
              <button @click="editModal = false" class="px-6 py-2.5 rounded-xl text-sm font-semibold text-slate-600 bg-white hover:bg-slate-50 border border-slate-200 transition-all shadow-sm">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div v-if="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-sm bg-white border border-rose-200 rounded-3xl shadow-2xl overflow-hidden">
            <div class="p-6 text-center">
              <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-rose-50 border border-rose-200 mx-auto mb-4">
              ...
              <p class="text-sm text-slate-500 mb-6">
                <span class="text-slate-700 font-semibold">{{ poolToDelete?.label || poolToDelete?.name }}</span> will be permanently removed. This cannot be undone.
              </p>
              <div class="flex gap-3">
                <button @click="deleteModal = false" class="flex-1 py-2.5 rounded-xl text-sm font-semibold text-slate-600 bg-white hover:bg-slate-50 border border-slate-200 transition-all shadow-sm">
                  Cancel
                </button>
                <button @click="deletePool" class="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-rose-500 to-rose-600 hover:from-rose-600 hover:to-rose-700 shadow-lg shadow-rose-500/20 transition-all hover:-translate-y-0.5">
                  Delete Pool
                </button>
              </div>
            </div>
    ...
    <!-- ===== CHEMICAL READINGS LOG MODAL ===== -->
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="readingsModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div class="absolute inset-0 bg-slate-900/70 backdrop-blur-sm" @click="readingsModal = false"></div>
          <div class="relative w-full max-w-5xl bg-white border border-slate-200 rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <!-- Header -->
            <div class="relative bg-gradient-to-r from-teal-50 via-cyan-50 to-slate-50 px-6 py-5 border-b border-slate-200 shrink-0">
              <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex items-start gap-4">
                  <div class="flex items-center justify-center w-14 h-14 rounded-3xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20 border border-teal-200 shadow-sm">
                    <svg class="w-6 h-6 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/>
                    </svg>
                  </div>
                  <div>
                    <h2 class="text-2xl font-bold text-slate-900">Chemical Readings Log</h2>
                    <p class="text-sm text-slate-500 mt-1">{{ readingsPool?.label || readingsPool?.name }} · #{{ readingsPool?.id }}</p>
                  </div>
                </div>
                <button @click="readingsModal = false" class="flex items-center justify-center w-10 h-10 rounded-2xl text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-all">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
              </div>

              <div class="mt-5 grid grid-cols-1 lg:grid-cols-[1.2fr_auto] gap-3 items-end">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label class="block text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500 mb-2">From</label>
                    <input v-model="readingsFilters.date_from" type="date" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-500/40" />
                  </div>
                  <div>
                    <label class="block text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500 mb-2">To</label>
                    <input v-model="readingsFilters.date_to" type="date" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-500/40" />
                  </div>
                </div>
                <button
                  @click="fetchChemicalReadings(1)"
                  :disabled="readingsLoading"
                  class="w-full sm:w-auto inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-teal-600 to-cyan-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-teal-500/20 hover:from-teal-700 hover:to-cyan-700 disabled:cursor-not-allowed disabled:opacity-50 transition-all"
                >
                  {{ readingsLoading ? 'Loading...' : 'Apply Filter' }}
                </button>
              </div>
            </div>

            <div class="flex-1 overflow-y-auto p-6 space-y-4">
              <div v-if="readingsLoading" class="space-y-4">
                <div v-for="i in 3" :key="i" class="h-32 rounded-3xl bg-slate-100 animate-pulse"></div>
              </div>

              <div v-else-if="chemicalReadings.length === 0" class="rounded-3xl border border-slate-200 bg-slate-50 p-10 text-center">
                <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-teal-100 text-teal-600">
                  <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
                </div>
                <h3 class="text-lg font-semibold text-slate-900 mb-2">No readings found</h3>
                <p class="text-sm text-slate-500">Try widening the date range or selecting another pool.</p>
              </div>

              <div v-else class="space-y-4">
                <article
                  v-for="reading in chemicalReadings"
                  :key="reading.id"
                  class="rounded-[28px] border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-lg"
                >
                  <div class="px-6 py-5 border-b border-slate-100">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                      <div>
                        <div class="text-sm font-semibold text-slate-900">{{ formatDateTime(reading.recorded_at) }}</div>
                        <div class="flex flex-wrap gap-3 text-xs text-slate-500 mt-2">
                          <span v-if="reading.work_order?.technician?.name">Technician: {{ reading.work_order.technician.name }}</span>
                          <span v-if="reading.work_order_id">Work Order #{{ reading.work_order_id }}</span>
                        </div>
                      </div>
                      <span
                        :class="[
                          'inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-semibold border',
                          reading.is_balanced
                            ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                            : 'bg-amber-50 text-amber-700 border-amber-200'
                        ]"
                      >
                        <span :class="['h-2.5 w-2.5 rounded-full', reading.is_balanced ? 'bg-emerald-500' : 'bg-amber-500']"></span>
                        {{ reading.is_balanced ? 'Balanced' : 'Needs Attention' }}
                      </span>
                    </div>
                  </div>
                  ...
                  <div class="px-6 py-5 space-y-5">
                    <div class="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-7 gap-3">
                      <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-center">
                        <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-semibold">pH</div>
                        <div class="mt-3 text-lg font-semibold text-slate-900">{{ reading.ph ?? '—' }}</div>
                      </div>
                      <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-center">
                        <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-semibold">Free CL</div>
                        <div class="mt-3 text-lg font-semibold text-slate-900">{{ reading.free_chlorine ?? '—' }}</div>
                      </div>
                      <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-center">
                        <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-semibold">Total CL</div>
                        <div class="mt-3 text-lg font-semibold text-slate-900">{{ reading.total_chlorine ?? '—' }}</div>
                      </div>
                      <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-center">
                        <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-semibold">Alkalinity</div>
                        <div class="mt-3 text-lg font-semibold text-slate-900">{{ reading.alkalinity ?? '—' }}</div>
                      </div>
                      <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-center">
                        <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-semibold">CYA</div>
                        <div class="mt-3 text-lg font-semibold text-slate-900">{{ reading.cya ?? '—' }}</div>
                      </div>
                      <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-center">
                        <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-semibold">Salt</div>
                        <div class="mt-3 text-lg font-semibold text-slate-900">{{ reading.salt ?? '—' }}</div>
                      </div>
                      <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-center">
                        <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-semibold">Clarity</div>
                        <div class="mt-3 text-lg font-semibold text-slate-900">{{ reading.water_clarity ? formatClarity(reading.water_clarity) : '—' }}</div>
                      </div>
                    </div>

                    <div v-if="reading.analysis && Object.keys(reading.analysis).length" class="space-y-3">
                      <div class="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Analysis</div>
                      <div class="grid gap-3">
                        <div
                          v-for="(item, key) in reading.analysis"
                          :key="key"
                          :class="['rounded-3xl border p-4 text-sm', getAnalysisStyle(item.status)]"
                        >
                          <div class="font-semibold text-slate-900 capitalize">{{ formatAnalysisKey(key) }}</div>
                          <div class="mt-1 text-slate-600">{{ item.message }}</div>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div v-if="readingsPagination.total > 0" class="shrink-0 px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-3">
              <span class="text-sm text-slate-500">
                {{ readingsPagination.total }} reading{{ readingsPagination.total === 1 ? '' : 's' }} total
              </span>
              <div class="flex items-center gap-2">
                <button
                  @click="fetchChemicalReadings(readingsPagination.current_page - 1)"
                  :disabled="readingsPagination.current_page <= 1 || readingsLoading"
                  class="rounded-2xl px-3 py-2 text-xs font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 transition"
                >
                  Previous
                </button>
                <span class="text-xs text-slate-500">Page {{ readingsPagination.current_page }} of {{ readingsPagination.last_page }}</span>
                <button
                  @click="fetchChemicalReadings(readingsPagination.current_page + 1)"
                  :disabled="readingsPagination.current_page >= readingsPagination.last_page || readingsLoading"
                  class="rounded-2xl px-3 py-2 text-xs font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 transition"
                >
                  Next
                </button>
...
const readingsModal = ref(false)
const selectedPool = ref(null)
const readingsPool = ref(null)
const poolToDelete = ref(null)
const isAdding = ref(false)

const chemicalReadings = ref([])
const readingsLoading = ref(false)
const readingsPagination = ref({ current_page: 1, last_page: 1, total: 0 })
const readingsFilters = reactive({
  date_from: '',
  date_to: '',
})
...
async function fetchChemicalReadings(page = 1) {
  if (!readingsPool.value?.id) return
  readingsLoading.value = true
  try {
    const params = new URLSearchParams({
      pool_id: readingsPool.value.id,
      date_from: readingsFilters.date_from || getDefaultDateFrom(),
      date_to: readingsFilters.date_to || getDefaultDateTo(),
      page: String(page),
    })
    const response = await apiClient.get(`/chemical-management/readings?${params}`)
    const payload = response.data
    chemicalReadings.value = payload?.data || []
    readingsPagination.value = {
      current_page: payload?.current_page || 1,
      last_page: payload?.last_page || 1,
      total: payload?.total || chemicalReadings.value.length,
    }
  } catch (error) {
    console.error('Error fetching chemical readings:', error)
    chemicalReadings.value = []
    readingsPagination.value = { current_page: 1, last_page: 1, total: 0 }
    alert(error.response?.data?.message || 'Failed to load chemical readings.')
  } finally {
    readingsLoading.value = false
  }
}

function openReadingsModal(pool) {
  readingsPool.value = pool
  readingsFilters.date_from = getDefaultDateFrom()
  readingsFilters.date_to = getDefaultDateTo()
  chemicalReadings.value = []
  readingsPagination.value = { current_page: 1, last_page: 1, total: 0 }
  readingsModal.value = true
  fetchChemicalReadings(1)
}

function getReadingMetrics(reading) {
  return [
    { label: 'pH', value: reading.ph ?? '—' },
    { label: 'Free Cl', value: reading.free_chlorine ?? '—' },
    { label: 'Total Cl', value: reading.total_chlorine ?? '—' },
    { label: 'Alkalinity', value: reading.alkalinity ?? '—' },
    { label: 'CYA', value: reading.cya ?? '—' },
    { label: 'Salt', value: reading.salt ?? '—' },
    { label: 'Clarity', value: reading.water_clarity ? formatClarity(reading.water_clarity) : '—' },
  ]
}
...
function getAnalysisStyle(status) {
  const map = {
