<template>
  <div class="min-h-screen bg-gray-50 p-6" ref="calendarContainer">
    ...
    <Transition name="toast">
      <div v-if="errorMessage" class="fixed top-6 right-6 z-50 flex items-center gap-3 bg-white border border-red-200 text-red-700 px-4 py-3 rounded-2xl shadow-lg">
    ...
    <!-- Empty State -->
    <div v-else-if="!initialLoading && calendarData.total_visits === 0 && calendarDays.every(d => d.visits.length === 0)" class="flex flex-col items-center justify-center min-h-[600px]">
      <div class="text-center">
        ...
        <div class="flex items-center justify-center gap-3 mt-4">
          <button @click="prevMonth" class="px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-sm">← Previous Month</button>
          <button @click="goToToday" class="px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-sm">Today</button>
          <button @click="nextMonth" class="px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-sm">Next Month →</button>
        </div>
      </div>
    </div>
    ...
    <div v-else>
      ...
      <div class="flex justify-between items-start mb-6">
        ...
        <div class="flex items-center gap-3">
          <button class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors shadow-sm flex items-center gap-2 text-sm font-medium">
      ...
      <div class="flex gap-6">
        <!-- Calendar Grid -->
        <div class="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 relative" :class="{ 'opacity-60 pointer-events-none': loading }">
          ...
          <div class="grid grid-cols-7 gap-1">
            ...
            >
              ...
              <div class="mt-1 space-y-1">
                <div
                  v-for="visit in day.visits.slice(0, 2)"
                  :key="visit.id"
                ...
                >
                  <div class="font-semibold truncate text-gray-800">{{ visit.pool?.label || visit.pool_name || '—' }}</div>
                  <div class="truncate text-gray-600">{{ visit.pool?.customer?.contact_name || visit.customer_name || '—' }}</div>
                  <div class="truncate text-gray-500 mt-0.5">{{ formatTimeWindow(visit.time_window_start, visit.time_window_end) }}</div>
                </div>
              ...
              >
                <div class="flex justify-between items-start gap-2">
                  <div class="min-w-0">
                    ...
                    <p class="text-sm text-gray-500 truncate">{{ visit.pool?.customer?.contact_name || visit.customer_name || '—' }}</p>
                  </div>
                  <span class="text-xs px-2 py-1 rounded-full font-medium flex-shrink-0 border" :class="getStatusColor(visit.status)">
        ...
        <div class="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="flex justify-between items-start">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap mb-2">
                  <span class="text-xs px-2.5 py-1 rounded-full font-semibold border" :class="getStatusColor(selectedEvent.status)">
                    {{ formatLabel(selectedEvent.status) }}
                  </span>
                  <span class="text-xs px-2.5 py-1 rounded-full font-semibold border" :class="getPriorityColor(selectedEvent.priority)">
          ...
          <div class="px-6 py-4 space-y-1 text-sm">
            <div class="flex justify-between py-2.5 border-b border-gray-50">
              ...
              <span class="font-semibold text-gray-900">{{ selectedEvent.pool?.customer?.contact_name || selectedEvent.customer_name || '—' }}</span>
            </div>
            <div class="flex justify-between py-2.5 border-b border-gray-50">
              <span class="text-gray-500 font-medium">Pool</span>
              <span class="font-semibold text-gray-900">{{ selectedEvent.pool?.label || selectedEvent.pool_name || '—' }}</span>
            </div>
            <div class="flex justify-between py-2.5 border-b border-gray-50">
              <span class="text-gray-500 font-medium">Technician</span>
              <span class="font-semibold text-gray-900">{{ selectedEvent.technician?.name || selectedEvent.technician_name || '—' }}</span>
            </div>
            <div class="flex justify-between py-2.5 border-b border-gray-50">
              <span class="text-gray-500 font-medium">Time Window</span>
              <span class="font-semibold text-gray-900">{{ formatTimeWindow(selectedEvent.time_window_start, selectedEvent.time_window_end) }}</span>
            </div>
            <div class="flex justify-between py-2.5 border-b border-gray-50">
          ...
          <div class="px-6 pb-6">
            <div class="flex flex-wrap gap-2">
              <button class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors text-sm font-medium flex-1">
                Start Visit
              </button>
              <button class="px-4 py-2 border border-gray-200 bg-white rounded-xl hover:bg-gray-50 transition-colors text-sm font-medium flex-1 text-gray-700">
                Reschedule
              </button>
              <button class="px-4 py-2 border border-gray-200 bg-white rounded-xl hover:bg-gray-50 transition-colors text-sm font-medium flex-1 text-gray-700">
                Change Tech
              ...
              <button class="px-4 py-2 border border-red-200 text-red-600 bg-white rounded-xl hover:bg-red-50 transition-colors text-sm font-medium flex-1">
                Cancel Visit
              </button>
            </div>
          </div>
...
<style scoped>
.calendar-grid { transition: all 0.2s ease; }
